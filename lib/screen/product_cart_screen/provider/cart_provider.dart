...
class CartProvider extends ChangeNotifier {
  ...
  // complete getCouponDiscountAmount
  ...
  Future<void> stripePayment({required void Function() operation}) async {
    try {
      Map<String, dynamic> paymentData = {
        ...
        "amount": getGrandTotal() * 100, // complete amount grand total // payment gateway need to be multiplyed by 100
        "currency": "usd",
  ...
  Future<void> razorpayPayment({required void Function() operation}) async {
    try {
      ...
      if (razorpayKey != null && razorpayKey != '') {
        var options = {
          'key': razorpayKey,
          'amount': getGrandTotal(), // complete amount grand total
          'name': "user",
          "currency": 'INR',
